...
class ExamplePostType {
	...
	public function init(): void {
		...
		add_action( 'carbon_fields_register_fields', array( $this, 'register_carbon_fields' ), 20 );
	...
	public function register_taxonomies(): void {
		...
		register_taxonomy( self::TAXONOMY_EXAMPLE_TYPE, array( self::POST_TYPE ), $args );
	}
			...
			->add_fields(
				array(
					Field::make( 'image', 'js_example_image', __( 'Beispiel Bild', 'plugin-boilerplate' ) )
						...
						->set_type( array( 'image/jpeg', 'image/png' ) )
						->set_value_type( 'id' ),
	...
	public function add_taxonomy_filter(): void {
		...
		$selected_example_type = $_GET[ self::TAXONOMY_EXAMPLE_TYPE ] ?? '';
	...
	public function filter_posts_by_taxonomy( \WP_Query $query ): void {
		...
		if ( ! is_admin() || ! $query->is_main_query() || self::POST_TYPE !== $typenow ) {
			return;
		}

		$example_type = $_GET[ self::TAXONOMY_EXAMPLE_TYPE ] ?? '';
	...
	public function add_example_post_type_to_dashboard( array $args ): array {
		$args['post_type'] = array( 'post', self::POST_TYPE );
		return $args;
	}
}
